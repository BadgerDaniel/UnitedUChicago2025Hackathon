...
import { ArrowUp, Bot, User, AlertCircle, CheckCircle, Brain, Zap, Search } from 'lucide-react';
import { agentApi } from '../services/agentApi';

interface Message {
  id: string;
  type: 'user' | 'assistant' | 'error' | 'info' | 'status';
  content: string;
  timestamp: Date;
  agent?: string;
  isTemporary?: boolean;
...
const ChatInterfaceWithStatus: React.FC<ChatInterfaceProps> = ({ isExpanded }) => {
  ...
  const quickSuggestions = [
    'Search for flights from ORD to LHR',
    'What events are happening in Chicago?',
    'Weather at Denver airport',
    'United Airlines stock performance',
  ];

  const thinkingMessages = [
    { icon: Brain, text: "Analyzing your request..." },
    { icon: Search, text: "Routing to specialized agents..." },
    { icon: Zap, text: "Processing with AI models..." },
    { icon: Bot, text: "Consulting multiple agents..." },
  ];
  ...
  const startStatusMessages = () => {
    let messageIndex = 0;
    statusMessageId.current = Date.now().toString();
    
    // Add initial status message
    const initialStatus: Message = {
      id: statusMessageId.current,
      type: 'status',
      content: thinkingMessages[0].text,
      timestamp: new Date(),
      isTemporary: true,
    };
    setMessages(prev => [...prev, initialStatus]);

    // Cycle through status messages
    statusInterval.current = setInterval(() => {
      messageIndex = (messageIndex + 1) % thinkingMessages.length;
      setMessages(prev => prev.map(msg => 
        msg.id === statusMessageId.current
          ? { ...msg, content: thinkingMessages[messageIndex].text }
          : msg
      ));
    }, 2000);
  };
  ...
  const stopStatusMessages = () => {
    ...
    if (statusMessageId.current) {
      setMessages(prev => prev.filter(msg => msg.id !== statusMessageId.current));
      statusMessageId.current = null;
    }
  };
  ...
  const getMessageIcon = (type: string, index?: number) => {
    switch (type) {
      case 'user':
      ...
      case 'status':
        const IconComponent = thinkingMessages[index || 0].icon;
        return <IconComponent className="w-4 h-4 animate-pulse" />;
      default:
    ...
    <div className="bg-white dark:bg-slate-900 rounded-lg shadow-lg border border-gray-200 dark:border-slate-700 h-full flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        {messages.map((message, index) => {
          const statusIndex = message.content ? thinkingMessages.findIndex(m => m.text === message.content) : 0;
              ...
              <div className={`flex max-w-[80%] ${message.type === 'user' ? 'flex-row-reverse' : 'flex-row'} items-start space-x-2`}>
                <div className={`w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 ${
                  message.type === 'user' 
                    ...
                    : message.type === 'status'
                    ? 'bg-gradient-to-br from-purple-500 to-pink-500 text-white mr-2 animate-pulse'
                    : 'bg-gray-200 dark:bg-slate-700 text-gray-600 dark:text-gray-300 mr-2'
                }`}>
                  {getMessageIcon(message.type, statusIndex)}
                </div>
                <div className={`rounded-lg p-3 border ${
                  message.type === 'user'
                    ...
                    : message.type === 'status'
                    ? 'bg-gradient-to-r from-purple-50 to-pink-50 dark:from-purple-900/20 dark:to-pink-900/20 text-purple-900 dark:text-purple-200 border-purple-300 dark:border-purple-800 animate-pulse'
                  ...
                  {!message.isTemporary && (
                    <div className={`text-xs mt-1 opacity-70 font-medium flex items-center justify-between ${
                      ...
                      message.type === 'status' ? 'text-purple-600 dark:text-purple-400' :
...
export default ChatInterfaceWithStatus;