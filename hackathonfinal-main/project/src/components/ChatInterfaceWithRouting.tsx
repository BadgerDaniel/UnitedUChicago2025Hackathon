...
const ChatInterfaceWithRouting: React.FC<ChatInterfaceProps> = ({ isExpanded }) => {
  ...
  const routingMessageIds = useRef<Set<string>>(new Set());
  const statusInterval = useRef<NodeJS.Timeout | null>(null);
  const currentMessageIndex = useRef<number>(0);
  ...
  const startRoutingMessages = (query: string) => {
    // Predict which agents will be involved
    const predictions = agentPredictor.predictAgents(query);
    const progressMessages = agentPredictor.getProgressMessages(predictions);
    
    currentMessageIndex.current = 0;
    
    // Add initial routing message
    const firstMessageId = Date.now().toString();
    routingMessageIds.current.add(firstMessageId);
    
    const initialMessage: Message = {
      id: firstMessageId,
      type: 'routing',
      content: progressMessages[0],
      timestamp: new Date(),
      isTemporary: true,
    };
    setMessages(prev => [...prev, initialMessage]);

    // Cycle through routing messages
    statusInterval.current = setInterval(() => {
      currentMessageIndex.current = (currentMessageIndex.current + 1) % progressMessages.length;
      const newMessage = progressMessages[currentMessageIndex.current];
      
      // Add new routing message
      const newMessageId = (Date.now() + currentMessageIndex.current).toString();
      routingMessageIds.current.add(newMessageId);
  ...
  const stopRoutingMessages = () => {
    if (statusInterval.current) {
      clearInterval(statusInterval.current);
      statusInterval.current = null;
    }
    
    // Remove all routing messages
    setMessages(prev => prev.filter(msg => !routingMessageIds.current.has(msg.id)));
    routingMessageIds.current.clear();
  };
  ...
  const handleSendMessage = async () => {
    ...
    // Start showing routing messages based on the query
    startRoutingMessages(userQuery);
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        {messages.map((message) => {
          ...
          if (message.type === 'routing' && message.content.includes(':')) {
            agentName = message.content.split(':')[0].replace('🧠', '').replace('✈️', '').replace('🌤️', '').replace('🎉', '').replace('📊', '').replace('📰', '').replace('🌐', '').replace('🔄', '').replace('✨', '').trim();
          }
            ...
            >
              <div className={`flex max-w-[80%] ${message.type === 'user' ? 'flex-row-reverse' : 'flex-row'} items-start space-x-2`}>
                <div className={`w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 ${
                  ...
                  {message.type === 'routing' ? getAgentIcon(agentName) : 
...
export default ChatInterfaceWithRouting;