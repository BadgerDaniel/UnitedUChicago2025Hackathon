...
import { ArrowUp, Bot, User, AlertCircle, CheckCircle, Loader } from 'lucide-react';
import { agentApiSSE, StreamEvent } from '../services/agentApiSSE';
import MarkdownMessage from './MarkdownMessage';

interface Message {
  id: string;
  type: 'user' | 'assistant' | 'error' | 'info' | 'thinking';
  content: string;
  timestamp: Date;
  agent?: string;
  isStreaming?: boolean;
...
const ChatInterfaceStreaming: React.FC<ChatInterfaceProps> = ({ isExpanded }) => {
  ...
  const checkConnection = async () => {
    const isConnected = await agentApiSSE.checkHealth();
      ...
      if (!streamingMessageId.current) {
        ...
        setMessages(prev => prev.map(msg => 
          msg.id === streamingMessageId.current 
            ? { ...msg, content: event.content || msg.content, agent: event.agent || msg.agent }
            : msg
        ));
      }
        ...
        setMessages(prev => prev.map(msg => 
          msg.id === streamingMessageId.current
            ? {
                ...msg,
                type: 'assistant',
                content: event.content || 'Response received',
                isStreaming: false,
              }
            : msg
        ));
  ...
  const handleSendMessage = async () => {
    ...
    try {
      const response = await agentApiSSE.sendMessageStreaming(inputValue, handleStreamEvent);
      
      if (response.success && response.response && !streamingMessageId.current) {
        // If we didn't get streaming events, add the response directly
            ...
            <div className={`flex max-w-[80%] ${message.type === 'user' ? 'flex-row-reverse' : 'flex-row'} items-start space-x-2`}>
              ...
              <div className={`rounded-lg p-3 border ${
                message.type === 'user'
                  ...
                  : message.type === 'thinking'
                  ? 'bg-yellow-50 dark:bg-yellow-900/20 text-yellow-900 dark:text-yellow-200 border-yellow-300 dark:border-yellow-800'
                ...
                <div className={`text-xs mt-1 opacity-70 font-medium flex items-center justify-between ${
                  ...
                  message.type === 'thinking' ? 'text-yellow-600 dark:text-yellow-400' :
