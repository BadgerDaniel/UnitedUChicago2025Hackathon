"""
Streamlined Task Manager for Travel Analysis Agent.
Removed Linux agent references and simplified for travel analysis use case.
"""
...
class AgentTaskManager(InMemoryTaskManager):
    """Task manager for the Travel Analysis Agent."""
    ...
    async def _run_streaming_agent(self, request: SendTaskStreamingRequest):
        """Run the travel analysis agent in streaming mode."""
        ...
        try:
            ...
            logger.error(f'Error during travel analysis streaming: {e}')
            await self.enqueue_events_for_sse(
                task_send_params.id,
                InternalError(
                    message=f'Error during travel analysis: {e}'
                ),
            )
    ...
    def _validate_request(
        ...
        """Validate incoming requests for travel analysis compatibility."""
        task_send_params: TaskSendParams = request.params
        if not utils.are_modalities_compatible(
            task_send_params.acceptedOutputModes,
            TravelAnalysisAgent.SUPPORTED_CONTENT_TYPES,
        ):
            logger.warning(
                'Unsupported output mode for travel analysis. Received %s, Support %s',
                task_send_params.acceptedOutputModes,
                TravelAnalysisAgent.SUPPORTED_CONTENT_TYPES,
            )
    ...
    async def on_send_task(self, request: SendTaskRequest) -> SendTaskResponse:
        """Handle travel analysis task requests."""
        ...
        try:
            logger.info(f'Processing travel analysis query: {query[:100]}...')
            agent_response = await self.agent.invoke(query, task_send_params.sessionId)
        except Exception as e:
            logger.error(f'Error invoking travel analysis agent: {e}')
            raise ValueError(f'Error processing travel query: {e}')
    ...
    async def on_send_task_subscribe(
        ...
        """Handle streaming travel analysis requests."""
        try:
            ...
            logger.error(f'Error in travel analysis SSE stream: {e}')
            print(traceback.format_exc())
            return JSONRPCResponse(
                id=request.id,
                error=InternalError(
                    message='An error occurred while streaming travel analysis'
                ),
            )
    ...
    async def _process_agent_response(
        ...
        """Process travel analysis agent response and update task store."""
        task_send_params: TaskSendParams = request.params
    ...
    def _get_user_query(self, task_send_params: TaskSendParams) -> str:
        ...
        if not isinstance(part, TextPart):
            raise ValueError('Only text queries are supported for travel analysis')
        return part.text
    ...
    async def send_task_notification(self, task: Task):
        ...
        logger.info(f'Sending travel analysis notification for task {task.id} => {task.status.state}')
    ...
    async def on_resubscribe_to_task(
        ...
        """Handle task resubscription for travel analysis."""
        ...
        try:
            ...
            logger.error(f'Error while reconnecting to travel analysis SSE stream: {e}')
            return JSONRPCResponse(
                id=request.id,
                error=InternalError(
                    message=f'An error occurred while reconnecting to travel analysis stream: {e}'
                ),
            )
