...
class FlightAgent:
    ...
    async def predict_price_changes(self, route: str, factors: Dict[str, Any]) -> Dict[str, Any]:
        """Predict how external factors will affect flight prices."""
        try:
            analysis_prompt = f"""
            Predict flight price changes for route {route} based on these factors:
            
            Weather Impact: {factors.get('weather_impact', 'None')}
            Event Impact: {factors.get('event_impact', 'None')}
            Seasonal Factors: {factors.get('seasonal', 'Normal')}
            Historical Trends: {factors.get('historical', 'Stable')}
            ...
            # Calculate predicted change based on external factors
            weather_factor = factors.get('weather_impact', {}).get('impact_score', 0) * 0.05
            event_factor = factors.get('event_impact', {}).get('impact_score', 0) * 0.08
            
            predicted_change = (weather_factor + event_factor) * 100  # Convert to percentage
            
            return {
                "route": route,
                "predicted_change_percent": min(predicted_change, 50),  # Cap at 50%
                "confidence_level": 75,
                "analysis": response.content,
                "factors_considered": list(factors.keys()),
                "recommendation": "Monitor prices closely" if predicted_change > 10 else "Normal booking practices"
            }
    ...
    async def find_best_deals(self, routes: List[Dict[str, str]], 
                            date_flexibility: int = 3) -> List[Dict[str, Any]]:
        """Find best flight deals considering date flexibility."""
        try:
            deals = []
            ...
            for route in routes:
                ...
                preferred_date = route.get("date", "")
                
                if not all([origin, destination, preferred_date]):
                    continue
                
                # Generate flexible dates
                flexible_dates = [preferred_date]  # Simplified
                
                flights = await self.search_flights(origin, destination, flexible_dates)
                ...
                if flights:
                    ...
                    deals.append({
                        ...
                        "best_date": best_flight.departure_date,
                        "savings_potential": "15%",  # Mock calculation
                        "flight_details": best_flight.dict()
                    })
    ...
    async def process_message(self, message: str, context: Dict[str, Any] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if "search" in message.lower() or "find" in message.lower():
                ...
                prediction = await self.predict_price_changes(f"{origin} to {destination}", external_factors)
    ...
    async def collaborate_with_agent(self, target_agent: str, message: str, 
        ...
        try:
            flight_context = {
                ...
                "price_predictions": shared_context.get("price_prediction", {}),
                "best_deals": shared_context.get("deals", []),
